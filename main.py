...
# from style import content_loss
...
def transfer(content_path, style_path, num_iters=1000, content_wt=1e3, style_wt=1e-2):
    model = get_model()
    for layer in model.layers:
        layer.trainable = False

    style_feats, content_feats = get_features(model, content_path, style_path)
    gram_style_feats = [gram_matrix(style_feat) for style_feat in style_feats]
    init_img = preprocess(load_img(content_path))
